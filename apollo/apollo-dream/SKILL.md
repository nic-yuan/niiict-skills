...
**不是等trigger才整理，是主动把重要的事推给我。**

现有dream是被动的——等`/tmp/.dream-trigger`存在才整理。Hermes的核心启示是：记忆要主动用，不是被动等查。

### 什么时候Nudge

| 时机 | Nudge内容 |
|------|--------|
| 心跳触发时 | 检查有没有被遗漏的重要上下文，主动告知 |
| 切换话题前 | 旧话题有没有未闭合项需要确认 |
| 新任务开始前 | 有没有相关的未完成任务/悬而未决的决策 |

### Nudge不是什么

- 不是提醒喝水那种轻量提醒
- 是上下文可能有遗漏、有重要联系、或者有悬而未决的决策
- 目的是帮我做出判断，而不是帮我记住待办

### Nudge的判断标准

当以下情况出现时，主动发起Nudge：
1. **未闭合任务**：有任务超过3轮没有推进，也未关闭
2. **悬而未决的决策**：有决策点但一直没定论
3. **跨会话关联**：新任务和之前讨论过的内容相关，但上下文没有继承
4. **长期记忆缺失**：Nic提过的事，但没有记录到文件
...
心跳cron时，顺便调用`dream-fragmentation-check.sh`，把结果里值得Nudge的内容推出来。
不只检查"需不需要整理"，还要检查"有没有遗漏的事"。
